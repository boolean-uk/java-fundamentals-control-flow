...
public class Extension extends ExtensionBase {
    ...
    public String timerStatus(int minutesLeftOnTimer) {
        if (minutesLeftOnTimer == 0) {
            return "The cake is ready!";
        } else if (minutesLeftOnTimer > 0) {
    ...
    public int estimatePrepTime(String[] ingredients, int prepTimeInMinutes) {
        if (prepTimeInMinutes < 1) {
            return ingredients.length * 2;
        } else {
            return ingredients.length * prepTimeInMinutes;
        }
    }
