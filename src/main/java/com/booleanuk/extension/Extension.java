...
public class Extension extends ExtensionBase {
    ...
    public int estimatePrepTime(String[] ingredients, int prepTimePer) {
        if (prepTimePer == 0) {
            prepTimePer = 2;
        }
        return ingredients.length *  prepTimePer;
    }
    ...
    public int calculateGramsOfSugar(String[] ingredients, int layers) {
        if (Arrays.stream(ingredients).anyMatch(x -> x.contains("sugar"))) {
            return layers * 100;
        } else {
            return 0;
        }
