...
public class Extension extends ExtensionBase {
    ...
    public int estimatePrepTime(String[] ingrs, int min){
        if(min == 0)
            return ingrs.length * 2;
        return ingrs.length * min;
    }
    ...
    public int calculateGramsOfSugar(String[] ingrs, int layers){
        for(String ingr: ingrs){
            if (ingr.equals("sugar"))
                return 100 * layers;
        }
        return 0;
    }
